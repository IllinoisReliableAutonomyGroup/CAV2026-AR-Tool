...
%
 The scenes are generated using \emph{FalconGym}~\cite{falcon} and \emph{Nerfstudio}~\cite{nerfstudio} which enable creation and semantic editing of 3DGS scenes.
%
...
%
The radius is a function of  the waypoint’s distance to neighboring gates. For example, for some controllers it is expected that the drone will be near the center of a gate when passing through it, but may deviate more when it is farther away from gates. Accordingly, the radius is set to be smaller when the waypoint is close to a gate and larger when it is farther away from both the preceding and subsequent gates.
For each point in this cylindrical neighborhood,  the camera's pose rotation matrix $R\in SO(3)$ is constructed such that the forward direction of the waypoint aligns with the vector pointing toward the center of the next gate, ensuring that the gate remains within the camera’s field of view. 
%
...
The network takes $64 \times 64$ RGB images as input and outputs a camera pose in $SE(3)$. 
Although relatively low-resolution, such images are commonly used in acrobatic drone racing and navigation tasks to achieve high frame-rate control under limited onboard computation~\cite{harris2021imitation,hanover2023adr_survey}. 
In this work, we focus on verifying the translational component of the estimated pose, represented as a vector in $\mathbb{R}^3$, corresponding directly to the camera’s position in the world coordinate frame.


\paragraph{Certifying GateNet with ART.} Let $P_i$ be a pose cell constructed from $P$ represented by the hyperrectangles $[p_l, p_u]$. Suppose the  abstract image from $[p_l, p_u]$ when passed through $GateNet$ using Crown produces an estimated pose interval $[\hat{p}_l, \hat{p}_u]$. The input pose cell $P_i$  is considered \emph{certified} to tolerance $\delta$ if
\[
...
\begin{figure}[h]
    ...
    \caption{\small Verification results across four track layouts: (a) circular, (b) U-turn, (c) right-angle, and (d) straight tracks. Green indicates certified regions; red denotes potential violations; blue indicates gates.}
    \label{fig:verify_tracks}
\end{figure}

Certified pose cells  are visualized in green in Fig~\ref{fig:verify_tracks}. The pose cells marked red indicate failure of certification, which suggests a potential violation of the $\delta$-accuracy requirement. Overall, our certification results are promising, with a significant portion of the pose space being certified. The uncertified cells make intuitive sense because these correspond to poses where the camera approaches the gate in a challenging angle.
For example in Fig.~\ref{fig:verify_abs_emp}(b), the gate is only partially visible in the top-right corner of the abstract images, making accurate pose estimation difficult. In contrast, Fig.~\ref{fig:verify_abs_emp}(a) shows a certified pose cell where the gate is clearly visible in the abstract image, leading to a reliable pose estimate.
%\marginpar{\sayan{show examples of figures from the failed cells?}}
The failed cells  could be used for further improving the training of the GateNet model or one may attempt to refine the pose cell partition to reduce over-approximation errors.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.48\textwidth]{figures/abs_viz_success.png}%
    \hspace{0.02\textwidth}%
    \includegraphics[width=0.48\textwidth]{figures/abs_viz_failure.png}
    
    \caption{\small Visualization of abstract images for (a) certifiable pose cell, and (b) failure pose cell. Gate is clearly visible in (a); in (b), it appears only at the top-right corner. For each subfigure (a) and (b), Top-left: sample concrete image from the pose cell; bottom-left/right: lower/upper bound abstract images; top-right: per-pixel difference heatmap between bounds.}
    \label{fig:verify_abs_emp}
\end{figure}



\paragraph{Hardware and Running Time.} All experiments are conducted on a workstation equipped with an NVIDIA GeForce RTX 4070 GPU with 16 GB of memory, using CUDA 12.2. Verifying the entire pose space across the four tracks requires approximately 10–11 hours of computation. This runtime is primarily constrained by GPU memory, which restrict the number of Gaussians that can be processed simultaneously. As a result, scenes with a larger number of Gaussians incur higher computational costs. In addition, the runtime grows as the number of pose space partitions increases. The size of the pose cells does not impact the runtime, except that larger cells  may lead to more conservative abstract images and analysis, and thus make the analysis inconclusive. 
